...
function splitHeroTitle(title: string) {
  ...
  // Last word gets the accent style
  const lastWord = parts.pop()!;
  // Split remaining into two roughly equal lines
...
export default function CityHero({ city }: { city: CityData }) {
  ...
  const { line1, line2, accent } = splitHeroTitle(city.heroTitle);
      ...
      <div className="max-w-[1400px] mx-auto px-5 lg:px-12 w-full pt-44 pb-12 flex-1 flex items-center relative z-2">
        <div className="grid lg:grid-cols-[1fr_1fr] gap-12 lg:gap-20 items-center w-full">
          ...
          <motion.div style={isMobile ? undefined : { y: textY }}>
            {/* Breadcrumb */}
            <motion.nav
              className="flex items-center gap-2 mb-10 text-[0.72rem] text-text-muted"
            ...
            >
              ...
              <span className="text-white/20">&rsaquo;</span>
              <span className="text-accent font-medium">{city.name}</span>
            </motion.nav>
            ...
            >
              ...
              <span className="text-[0.72rem] uppercase tracking-[3px] text-text-muted font-medium">
                Agence Web Cr&eacute;ative &mdash; {city.name}
              </span>
            </motion.div>

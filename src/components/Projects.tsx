      ...
      <div className="max-w-[1400px] mx-auto px-5 py-[100px] lg:px-12 relative z-2">
        <ScrollReveal>
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-14">
            ...
            <motion.a
              href="#"
              className="text-[0.82rem] text-accent font-semibold inline-flex items-center gap-2"
              whileHover={{ x: 4 }}
            >
              Voir tous les projets
              <svg viewBox="0 0 24 24" className="w-4 h-4 stroke-accent fill-none stroke-2"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
            </motion.a>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal>
          <motion.div
            className="rounded-2xl overflow-hidden relative group cursor-pointer mb-4"
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 lg:p-10">
              <div className="flex flex-wrap gap-2 mb-3">
                {featured.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-[0.7rem] font-medium text-white/80">
                    {tag}
                  </span>
                ))}
              </div>
              <h3 className="text-[1.3rem] lg:text-[1.8rem] font-bold text-white">{featured.title}</h3>
              <p className="text-[0.85rem] text-white/50 mt-1.5 max-w-[500px]">{featured.desc}</p>
            </div>
          </motion.div>
        </ScrollReveal>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  <div className="flex gap-1.5 mb-2">
                    {project.tags.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 rounded-full bg-white/10 backdrop-blur-sm text-[0.65rem] text-white/70">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <h4 className="text-[0.95rem] font-bold text-white">{project.title}</h4>
                  <p className="text-[0.75rem] text-white/40 mt-0.5">{project.desc}</p>
                </div>
              </motion.div>
            </ScrollReveal>
          ))}
